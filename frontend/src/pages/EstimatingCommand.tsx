/* ============================================================
   EstimatingCommand.tsx — O'Neill Elite Executive Dashboard
   ------------------------------------------------------------
   2026 Estimating Command Center with Sidebar Layout
   - Fixed sidebar with O'Neill branding
   - 6 KPI Cards with goal tracking
   - 3 Tables (Bidding, Pending, Lost)
   - Excel/CSV upload in sidebar
  ...
  return (
    <aside 
      ...
      style={{ 
        width: 260, 
        background: "#00205B",
        borderRight: "4px solid #009A44"
      }}
    >
      {/* Logo */}
      <div className="p-6 text-center">
        <div className="bg-white p-3 rounded mb-6">
        ...
        <nav className="space-y-2">
          <a 
            href="#" 
            className="flex items-center gap-3 px-4 py-3 text-white bg-white/10 rounded-lg font-bold text-xs uppercase"
          >
            📊 Dashboard Home
          </a>
...
export function EstimatingCommand() {
  ...
  // Auto-open wizard when navigating to /estimating/new
  useEffect(() => {
    if (location.pathname === "/estimating/new") {
      setWizardOpen(true);
    }
  }, [location.pathname]);
...
export default EstimatingCommand;
