...
export default function ExecutionPipeline() {
  ...
  useEffect(() => {
    fetchProjects(PipelineType.EXECUTION);
    fetchMetrics(PipelineType.EXECUTION);
  }, [fetchProjects, fetchMetrics]);

  const handleRefresh = async () => {
    toast.info('Refreshing pipeline data...');
    await fetchProjects(PipelineType.EXECUTION);
    await fetchMetrics(PipelineType.EXECUTION);
    toast.success('Pipeline data refreshed');
  };

  const handleExport = () => {
    const projects = getFilteredProjects();
    exportToCSV(projects, 'execution_pipeline');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-orange-50 to-slate-100">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-3xl font-bold text-slate-900">Execution Pipeline</h1>
                </div>
                <p className="text-slate-600 mt-1">
                  Mobilization → Foundation → Structure → MEP → Finishes → Commissioning
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
      ...
      <div className="max-w-7xl mx-auto px-8 py-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Construction Execution Stages</CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <div className="flex items-center justify-center h-96">
                <div className="text-center">
                  ...
                  <p className="text-slate-600">Loading execution projects...</p>
                </div>
              </div>
            ) : (
              <KanbanBoard 
